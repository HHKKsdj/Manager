...
//        self.navigationItem.title = "注册账号"
    ...
    func setUI() {
        ...
        linkButton.setTitle("没有邮箱？点击注册", for: .normal)
        linkButton.setTitleColor(.systemTeal, for: .normal)
        linkButton.titleLabel?.font = UIFont.systemFont(ofSize: 15)
        linkButton.addTarget(self, action: #selector(link(sender:)), for: .touchUpInside)
    ...
    @objc func link (sender:UIButton) {
        let url = NSURL(string: "https://mail.fzu.edu.cn/register/register.jsp")! as URL
